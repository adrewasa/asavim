...
function! nerdtree#ui_glue#createDefaultBindings()
    ...
    "下面是自己自定义的命令
    "gl
    call NERDTreeAddKeyMap({ 'key': g:NERDTreeMapGoLocation, 'scope': "Node", 'callback': s."openExplorer" })
...
function! s:openInNewTab(target)
    "如果是一个文件则在新标签页里面打开同事仍然保持着树打开的状态
    if !a:target.path.isDirectory
        call a:target.activate({'where': 't','keepopen':1})
    else
