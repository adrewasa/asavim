...
function! s:TreeDirNode._initChildren(silent)
    ...
    if version >= 703
        " 该命令为取得指定路径之下的所有文件，如果给定的不是一个目录前缀则不会进行返回
...
"打开目录所在的文件夹
function! s:TreeDirNode.openExplorer()
    let path = self.path.str({})
    let cmd = g:NERDTreeMapGoLocationCmd . path
    call system(cmd)
endfunction
