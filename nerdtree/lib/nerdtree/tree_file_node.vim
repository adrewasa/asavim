...
function! s:TreeFileNode._renderToString(depth, drawText)
    ...
    if self.path.isDirectory ==# 1 && self.isOpen ==# 1
        ...
        " 如果为目录的时候这是一个dir_node这时候才会定义该方法，其实就是返回经过一定规则定义的子
        " 节点的集合，但是子节点是什么时候进行创建的呢?
...
function! s:TreeFileNode.openExplorer()
    "在这里连目录节点的情况也考虑了，这样目录节点就不需要再写一遍该代码
    let s:path = ""
    if self.path.isDirectory
        let s:path = self.path.str({})
    else
        let s:path = self.parent.path.str({})
    endif
    let cmd = g:NERDTreeMapGoLocationCmd . s:path
    call system(cmd)
endfunction
